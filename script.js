...
function createTimestamp() {
    ...
    span.innerHTML = new Date().toLocateTimeString();
    statusOutput.appendChild(span);

    
  const br = document.createElement("br");
  statusOutput.appendChild(br);
}
...
function toggleStatus(e) {
    ...
    const isVisable = !statusOutput.classList.contains("hidden");
...
function startFlashing() {
  ...
  intervalId = setInterval(() => {
    elapsedSeconds++;
    stopwatchDisplay.textContent = formatTime(elapsedSeconds);
     stopwatchDisplay.classList.toggle("hidden");
  }, 500);
}
